...
public class IntegerSliderListEntry extends AbstractSliderListEntry<Integer, IntegerSliderListEntry.IntegerSliderListCell, IntegerSliderListEntry> {
    private static final Function<Integer, Text> DEFAULT_TEXT_GETTER = value -> new LiteralText(String.format("Value: %d", value));

    public IntegerSliderListEntry(Text fieldName, int minimum, int maximum, List<Integer> value, boolean defaultExpanded, Supplier<Optional<Text[]>> tooltipSupplier, Consumer<List<Integer>> saveConsumer, Supplier<List<Integer>> defaultValue, int cellDefaultValue, Text resetButtonKey, boolean requiresRestart, boolean deleteButtonEnabled, boolean insertInFront) {
